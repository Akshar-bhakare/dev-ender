...
import { hostEligibilityGuard } from '../../middleware/host-eligibility.middleware.js';
import * as EventsController from './events.controller.js';

export async function eventsRoutes(fastify: FastifyInstance) {
  fastify.post('/', { preHandler: [requireAuth, hostEligibilityGuard] }, EventsController.createEvent);
  fastify.patch('/:eventId', { preHandler: [requireAuth] }, EventsController.updateEvent);
  fastify.post('/:eventId/publish', { preHandler: [requireAuth, hostEligibilityGuard] }, EventsController.publishEvent);
